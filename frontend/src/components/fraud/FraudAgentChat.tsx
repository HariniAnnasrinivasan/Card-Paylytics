...
export default function FraudAgentChat() {
    const [isOpen, setIsOpen] = useState(false);
    const [input, setInput] = useState('');
    const [messages, setMessages] = useState<Message[]>([
        { role: 'assistant', content: 'Hello! I am your Fraud Intelligence Agent. I can help you analyze risk patterns and identify suspicious activity. What would you like to investigate?' }
    ]);
    ...
    const handleSend = async () => {
        ...
        try {
            const res = await api.post('/fraud/agent', { prompt: userMsg });
            setMessages(prev => [...prev, { role: 'assistant', content: res.data.response }]);
        } catch (err: any) {
            console.error('Fraud Agent error:', err);
            setMessages(prev => [...prev, { role: 'assistant', content: 'I am sorry, my analytical engine is currently processing a high volume of risk data. Please try again later.' }]);
        } finally {
            setLoading(false);
        }
    };
                ...
                <div className="absolute bottom-20 right-0 w-[400px] h-[550px] bg-[#0f0e2d]/90 border border-purple-500/30 rounded-3xl shadow-2xl flex flex-col overflow-hidden backdrop-blur-xl animate-in slide-in-from-bottom-5 duration-300">
                    ...
                    <div className="p-5 bg-gradient-to-r from-purple-600/30 to-orange-500/10 border-b border-purple-500/20 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-white font-bold text-sm tracking-tight">Fraud Intelligence Agent</p>
                                <div className="flex items-center gap-1.5 mt-0.5">
                                    <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>
                                    <span className="text-purple-300 text-[10px] font-bold uppercase tracking-widest font-mono">Secure Engine</span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="p-5 bg-white/5 border-t border-purple-500/10">
                        <div className="relative group">
                            <input
                                ...
                                placeholder="Analyze threat vectors..."
                        ...
                        <p className="text-[10px] text-center text-purple-400/40 mt-3 font-medium tracking-tight uppercase">Visa Analytics Infrastructure • us-east-2</p>
                    </div>
                </div>
            )}
        </div>
